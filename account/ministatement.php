<?php 
	...
	{	
		...
		$query_withdraw = "SELECT * FROM $mysql_transaction_withdraw WHERE `acc_no`='$acc_number' ORDER BY `dow` DESC LIMIT 5";
		$result1 = $conn->query($query_withdraw);

		$query_transfer = "SELECT * FROM $mysql_transaction_transfer WHERE `acc_no`='$acc_number' ORDER BY `dot` DESC LIMIT 5";
		$result2 = $conn->query($query_transfer);
