	...
	else
	{
			if(!empty($_POST['amt'])||!empty($_POST['pass_key']))
			{
				...
				if(is_numeric($acc_no))
				{
					...
					{
						$query = "SELECT * FROM $mysql_tb WHERE `acc_no`='$acc_no'";
									...
									if($password==$pass)
									{
										if($bal>='100')
										{
											if($amount<=$bal)
											{
												...
												if($own_balance>='100')
												{
														$query_withdraw = "UPDATE 	$mysql_tb SET `balance`='$own_balance' WHERE `acc_no`='$acc_no'";
														$result_withdraw = $conn->query($query_withdraw);
														...
														//	echo "<center> <p style='padding: 5px;background-color:red;'> Successfully Withdrawed from Account.</p>	</center>";
																	...
																	$query_5 = "INSERT INTO $mysql_transaction_withdraw(`acc_no`,`wid`) VALUES('$acc_no','$transfer_text');";
																		
																	if($conn->query($query_5))
																	{
